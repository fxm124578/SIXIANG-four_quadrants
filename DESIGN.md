...
> 状态：方案已按本文落地（2026-08 至 2026-09）；正文以当时设计（`web/app.html` 单文件内联）为基线，保持历史原貌。
> 实现演进（v2.0.0，2026-09-08）：`app.html` 已拆分为 `src/web/index.html` + `app.css` + `app.js`，由 `webview_main.py` 内置本地静态服务加载（外置 `web/` 优先、`_internal/` 兜底，改文件即生效）；入口新增单实例判定，窗口常驻托盘并支持全局热键。打包分发由 onefile 改为 onedir 目录 + Inno Setup。以当前代码为准。
> 日期：2026-08-25（初始方案）
